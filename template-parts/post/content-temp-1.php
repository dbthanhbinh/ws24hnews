<?php
	...
	$posCount = 0;
	$postCount = 0;
	if(isset($args['post_count']) && $args['post_count'] > 1){
		$hasPostCount = true;
		$postCount = $args['post_count'];
		if(isset($args['pos']) && $args['pos']) {
			$posCount = $args['pos'];
		}
		
		if(isset($args['pos']) && $args['pos'] == 1) {
			$isFirstPos = true;
			$cols = 712;
		} else {
			$cols = 2;
...
<article class="<?= $cardColClass ?>">
	...
	<div class="entry-content <?= $archiveDate ? HAS_BADGE_CLASS : '' ?>">
		...
		<div class="entry-summary">
			<?= get_excerpt($isFirstPos ? 170 : 70, $archiveReadMore); ?>
		</div>
...
<?php
	if($hasPostCount && $isFirstPos) {
		echo '<div class="'.getColsLayout($isGrid, 512).'"> <div class="row">';
	}

	if($hasPostCount && ($posCount) == $postCount) {
		echo '</div></div>';
	}
